...
│  │  Tables: test_suites, test_cases, runs, results, app_settings│   │
│  └──────────────────────────────────────────────────────────────┘   │
└──────────────────────────────────────────────────────────────────────┘
...
5 tables: `test_suites`, `test_cases`, `runs`, `results`, `app_settings`
...
A **singleton table** (single row) that was originally used for server-side key storage. The project evolved to a BYOK architecture where keys are sent via headers, making this table largely vestigial. It's still used for the `GET /api/settings` endpoint which reports server-side key status.
...
- `test_get_settings_creates_row`: Settings auto-creation
- `test_update_settings`: Save API key
- `test_update_settings_partial`: Partial update preserves other keys
...
A: 5 — test_suites, test_cases, runs, results, app_settings.
...
**3. Explain the BYOK security architecture in detail.**
A: Keys are stored in the browser's `localStorage` — never sent to the server for storage. When an evaluation is triggered, `buildKeyHeaders()` reads localStorage and attaches keys as custom HTTP headers (X-Gemini-Key, etc.). The backend's `get_resolved_keys()` extracts these headers, merges with server env vars (headers take priority), and uses them for that request only. After the request completes, the keys exist only in Python's function scope and are garbage collected.
...
**30. What happens to `app_settings` in the current BYOK architecture?**
A: It's a vestigial table from when keys were stored server-side. The current `GET /api/settings` endpoint doesn't actually query this table — it reads from environment variables via `get_env_key_status()`. The table exists but is unused.
...
**40. What is the purpose of the `onupdate=func.now()` in AppSettings?**
A: Automatically updates the `updated_at` timestamp whenever the row is modified, tracking the last change time without explicit code.
...
A: (1) Set up PostgreSQL on Render. (2) Run Alembic migration to create schema. (3) Write a one-time data migration script using SQLAlchemy to read from SQLite and write to PostgreSQL. (4) Update `DATABASE_URL` env var. (5) Restart backend. Since the app auto-creates tables at startup, this is straightforward.
...
18. What is a migration? → A versioned change to the database schema. Arbiter includes Alembic for this.
...
20. How would you implement database migrations with zero downtime? → Use Alembic with backward-compatible migrations (add columns as nullable, deploy new code, then add constraints).
